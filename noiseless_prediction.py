...
def estimate_next_pos(measurement, OTHER=None):
    ...
    if not OTHER:  # this is the first measurement
        OTHER = []
        OTHER.append(False)  # place holder for the step
        OTHER.append(False)  # place holder for the angle
        OTHER.append(False)  # place holder for the heading
        OTHER.append(measurement)
        xy_estimate = measurement
    else:
        if not (OTHER[0] and OTHER[1]):
            OTHER.append(measurement)
            if not OTHER[0]:
                step = distance_between(OTHER[-1], OTHER[-2])
                OTHER[0] = step
                xy_estimate = measurement
            if len(OTHER) > 5 and not OTHER[1]:  # have three measurements stored and angle not found
                angle1 = atan2(OTHER[-2][1] - OTHER[-3][1], OTHER[-2][0] - OTHER[-3][0])
                angle2 = atan2(OTHER[-1][1] - OTHER[-2][1], OTHER[-1][0] - OTHER[-2][0])
                angle = abs(angle1 - angle2)
                OTHER[1] = angle_trunc(angle)
                OTHER[2] = angle2
                OTHER[2] = angle_trunc(OTHER[2])

        if OTHER[0] and OTHER[1]:  # if step and angle is found
            OTHER[-1] = measurement
            OTHER[2] += OTHER[1]
            OTHER[2] = angle_trunc(OTHER[2])
            x = OTHER[-1][0] + OTHER[0] * cos(OTHER[2])
            y = OTHER[-1][1] + OTHER[0] * sin(OTHER[2])
            xy_estimate = [x, y]
